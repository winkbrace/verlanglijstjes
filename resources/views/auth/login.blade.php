    ...
    <x-auth-card title="Inloggen">
        ...
        <form method="POST" action="{{ route('login') }}">
            ...
            <div>
                ...
                <x-input id="name" class="block mt-0 sm:mt-1 w-full" type="text" name="name" :value="old('name')" required autofocus />
            </div>

            <!-- Password -->
            <div class="mt-4">
                <x-label for="password" :value="__('Wachtwoord')" />
                <x-input id="password" class="block mt-0 sm:mt-1 w-full" type="password" name="password" required autocomplete="current-password" />
            </div>

            <!-- Remember Me -->
{{--            <div class="block mt-4">--}}
{{--                <label for="remember_me" class="inline-flex items-center">--}}
{{--                    <input id="remember_me" type="checkbox" class="rounded-sm border-gray-300 text-indigo-600 shadow-xs focus:border-indigo-300 focus:ring-3 focus:ring-indigo-200/50" name="remember">--}}
...
{{--                @endif--}}
        ...
        <div>
            <a href="{{ route('oauth.google.redirect') }}" class="btn w-full block text-center bg-blue-100 px-3 py-2 shadow-xs border rounded-md text-blue-900">
